...
def platforms():
    ...
    platforms = Platform.query.order_by(Platform.weight.desc(), Platform.name).all()
...
def add_platform():
    if request.method == 'POST':
        ...
        internal_url = request.form.get('internal_url')
        description = request.form.get('description')
        ...
        internal_url = internal_url.strip() if internal_url else None

        # 调试信息
        print(f"处理后的URL数据: url='{url}', internal_url='{internal_url}'")

        # 检查是否至少提供了一个URL
        if not url and not internal_url:
            flash('外网URL和内网URL至少需要填写一个', 'danger')
            return redirect(url_for('admin.add_platform'))
...
def edit_platform(id):
    ...
    if request.method == 'POST':
        ...
        # 调试信息
        print(f"编辑后处理的URL数据: url='{url}', internal_url='{internal_url}'")
        ...
        platform.name = name
        platform.url = url
        platform.internal_url = internal_url
        platform.description = description
        platform.project_username = project_username
        platform.project_password = project_password
        platform.admin_username = admin_username
